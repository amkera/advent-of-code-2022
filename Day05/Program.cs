// Haven't used stacks before so watched this video: https://www.youtube.com/watch?v=fShdLAebYG4&list=UULFB6Td35bzTvcJN_HG6TLtwA&index=15
...
for (var instructionId = divider + 1; instructionId < lines.Length; instructionId++)
{
 ...
 var destinationStackId = int.Parse(parts[5]) - 1;

 for (var i = 0; i < numberOfCrates; i++)
 {
  var crate = stacks[sourceStackId].Pop();
  stacks[destinationStackId].Push(crate);
 }
}
